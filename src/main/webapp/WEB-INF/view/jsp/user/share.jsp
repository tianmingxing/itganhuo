					...
					<fieldset>
						...
						<div class="markdown_in_editor">
							...
							<div class="editor_buttons">
								<button id="submit_btn" type="button" class="btn btn-primary" onclick="submitForm();">提交</button>
							</div>
						</div>
					</fieldset>
				</form>
...
<script type="text/javascript">
	...
	function affix() {
		...
		if (project == null || project == '') {
			...
			var subject = jQuery('#j_label').val(), str = '';
			...
			jQuery('#j_label').val(str);
			jQuery('#j_label_txt').append('<span class="label">' + project + '</span>&nbsp;');
			jQuery("#project").val('');
		}
	}
	...
	function checkForm() {
		...
		var title = jQuery('#create_topic_form [name=title]').val(), j_label = jQuery('#j_label').val();
	...
	function submitForm() {
		...
		$.post('<%=path %>/user/share', $('#create_topic_form').serialize(), function(data){
			if (data.status == '0000') {
