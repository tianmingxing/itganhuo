		...
		<div id="add_dialog">
			<form id="add_form">
				<table style="width: 100%">
					<tr>
						<td>角色名称：</td>
						<td><input type="text" name="role" class="easyui-validatebox" data-options="required:true">
						</td>
						<td>角色描述：</td>
...
<script type="text/javascript">
	...
	$(function(){
		$('#list_datagrid').datagrid({
			loadMsg : '数据加载中...',
			url : '<%=path%>/admin/getRolesListJson',
			columns : [ [ {
				field : 'id',
				title : '编号',
				width : 100
            	...
            	$("#add_dialog").dialog({
					...
					handler : function() {
						if ($('#add_form').form('validate')) {
							$.ajax({
								type : "POST",
								url : "${pageContext.request.contextPath}/admin/updateRoles",
			...
			buttons : [ {
				...
				handler : function() {
					if ($('#add_form').form('validate')) {
						 $.ajax({
							type : "POST",
							url : "${pageContext.request.contextPath}/admin/addRoles",
